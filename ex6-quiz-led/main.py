...
button_mapping = {'A': 14, 'B': 5, 'C': 12, 'D': 4}
buttons = {key: machine.Pin(pin, machine.Pin.IN, machine.Pin.PULL_UP) for key, pin in button_mapping.items()}
...
def send_answer(addr, id, choice):
    # response = requests.get(f'http://{addr}/random').text   # Random reply endpoint used for testing
